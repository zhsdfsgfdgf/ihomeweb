...
type House struct {
	...
	Facilities      []*Facility   `orm:"reverse(many)" json:"facilities"`                    //反向多对多,房屋设施   与设施表进行关联
...
/* 区域信息 table_name = areea */ //区域信息是需要我们手动添加到数据库中的
type Area struct {
...
type Facility struct {
	...
	Houses []*House `orm:"rel(m2m)"` //多对多,都有哪些房屋有此设施  与房屋表进行关联的
}
...
func init() {
	...
	orm.RegisterDataBase("default", "mysql", "root:zhaoheng@tcp("+utils.G_mysql_addr+":"+utils.G_mysql_port+")/go1micro?charset=utf8", 30)
